...
print_organic <- function(x) {
	...
	cli::cli_text(cat("    "), "{.strong Std. error}: {round(calc_stderror(x$estimates$eif_ode), 4)}")
	cli::cli_text(cat("        "), "{.strong 95% CI}: ({round(calc_ci(x$estimates$ode, x$estimates$eif_ode)[1], 4)}, {round(calc_ci(x$estimates$ode, x$estimates$eif_ode)[2], 4)})")
	cli::cli_h3("{.emph Organic Indirect Effect}")
	cli::cli_text(cat("      "), "{.strong Estimate}: {round(x$estimates$oie, 4)}")
	cli::cli_text(cat("    "), "{.strong Std. error}: {round(calc_stderror(x$estimates$eif_oie), 4)}")
	cli::cli_text(cat("        "), "{.strong 95% CI}: ({round(calc_ci(x$estimates$oie, x$estimates$eif_oie)[1], 4)}, {round(calc_ci(x$estimates$oie, x$estimates$eif_oie)[2], 4)})")
}
...
print_ri <- function(x) {
	...
	cli::cli_text(cat("    "), "{.strong Std. error}: {round(calc_stderror(x$estimates$eif_ride), 4)}")
	cli::cli_text(cat("        "), "{.strong 95% CI}: ({round(calc_ci(x$estimates$ride, x$estimates$eif_ride)[1], 4)}, {round(calc_ci(x$estimates$ride, x$estimates$eif_ride)[2], 4)})")
	cli::cli_h3("{.emph Randomized Indirect Effect}")
	cli::cli_text(cat("      "), "{.strong Estimate}: {round(x$estimates$riie, 4)}")
	cli::cli_text(cat("    "), "{.strong Std. error}: {round(calc_stderror(x$estimates$eif_riie), 4)}")
	cli::cli_text(cat("        "), "{.strong 95% CI}: ({round(calc_ci(x$estimates$riie, x$estimates$eif_riie)[1], 4)}, {round(calc_ci(x$estimates$ride, x$estimates$eif_riie)[2], 4)})")
}
