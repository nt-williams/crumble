...
create_folds <- function(data, V, id, strata) {
	if (missing(strata)) {
		if (is.na(id)) {
			folds <- make_folds(data, V = V)
		} else {
			folds <- make_folds(data, cluster_ids = id, V = V)
		}
	...
	if ((is.na(id) | length(unique(data[[id]])) == nrow(data)) & binomial) {
		...
		folds <- make_folds(data, V = V, strata_ids = strata)
	} else {
		if (is.na(id)) folds <- make_folds(data, V = V)
		else folds <- make_folds(data, cluster_ids = data[[id]], V = V)
	}
